...
// Global Spotify auth token with expiration
let spotifyAccessToken = '';
let spotifyTokenExpiry = 0;

// Function to authenticate with Spotify and cache token
async function getSpotifyAccessToken(forceRefresh = false) {
    // Return cached token if still valid
    if (spotifyAccessToken && spotifyTokenExpiry > Date.now() && !forceRefresh) {
    ...
    try {
        ...
        const logMsg = forceRefresh ? "⏰ Token refreshed" : "🔑 Token obtained";
        console.log(`${logMsg}: expires in ${data.expires_in}s, token: ${spotifyAccessToken.substring(0, 10)}...`);
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith('rr_')) {
            ...
            if (!role) {
                // FIX: interaction was never deferred, so it must use .reply(), not .editReply()
        ...
        // QUIZ SUBMISSION REVIEW (Approve / Deny / Ban Creator)
        // FIX: these buttons were created in /quiz create but had no handler at all,
        // so clicking them previously did nothing and the interaction just failed silently.
        if (interaction.customId.startsWith('qapp_')) {
            if (!isAdmin) return interaction.reply({ content: "❌ Permission Denied.", ephemeral: true });

            const match = interaction.customId.match(/^qapp_(approve|deny|ban)_(.+)_(\d+)$/);
            if (!match) {
                return interaction.reply({ content: "❌ Could not parse this quiz submission button.", ephemeral: true });
            }
            const [, action, quizName, creatorId] = match;

            if (!db.customQuizzes || !db.customQuizzes[quizName]) {
                return interaction.reply({ content: "❌ That quiz no longer exists.", ephemeral: true });
            }

            const idx = db.customQuizzes[quizName].findIndex(q => q.creator === creatorId && q.approved === false);

            let resultText;
            if (action === 'approve') {
                if (idx > -1) db.customQuizzes[quizName][idx].approved = true;
                resultText = `✅ Question approved by ${interaction.user.tag}.`;
            } else if (action === 'deny') {
                if (idx > -1) db.customQuizzes[quizName].splice(idx, 1);
                resultText = `❌ Question denied by ${interaction.user.tag}.`;
            } else if (action === 'ban') {
                if (idx > -1) db.customQuizzes[quizName].splice(idx, 1);
                if (!db.quizBanned) db.quizBanned = [];
                if (!db.quizBanned.includes(creatorId)) db.quizBanned.push(creatorId);
                resultText = `🚫 <@${creatorId}> has been banned from creating quizzes by ${interaction.user.tag}.`;
            }
    ...
    if (interaction.isChatInputCommand()) {
        try {
            ...
            if (commandName === 'mute' && isTrial) {
                ...
                // FIX: guild.members.cache may not contain this member (large guilds don't cache
                // everyone), which made `.timeout(...)` throw on an undefined member. Fetch instead.
                const targetMember = await guild.members.fetch(target.id).catch(() => null);
                if (!targetMember) return interaction.editReply("❌ Could not find that member in this server.");

                await targetMember.timeout(m * 60000, reason);
            ...
            if (commandName === 'ship') {
                ...
                if (!u1 || !u2) {
                    // FIX: interaction.channel.members does not exist for regular text channels
                    // in discord.js v14 (only voice/thread channels have `.members`); reading it
                    // threw "Cannot read properties of undefined (reading 'filter')". Use the
                    // guild's member cache instead.
                    const allMembers = await getCachedMembers(interaction.guild).catch(() => interaction.guild.members.cache);
                    const nonBotMembers = allMembers.filter(m => !m.user.bot);
                    const randoms = nonBotMembers.random(2);
                    if (!randoms || randoms.length < 2) {
                        return interaction.editReply("❌ Not enough members in this server to make a match!");
                    }
            ...
            if (commandName === 'staffstats' && options.getSubcommand() === 'leaderboard') {
                ...
                const topWarns = Object.entries(db.notes || {})
                    ...
                    .slice(0, 5)
                    .map(([id, notesArr], i) => `**${i + 1}.** <@${id}>: \`${(notesArr || []).length}\` warnings`)
            ...
            if (commandName === 'reset-levels') {
                ...
                await interaction.editReply(`🗄️ **Database Syncing...** \`Connecting to OsQarek's Universe...\``);
